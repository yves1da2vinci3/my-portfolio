...
function Page() {
  ...
  return (
    <div
      className={` ${
        show ? 'h-screen' : 'h-auto'
      } px-3 overflow-hidden  relative flex flex-col items-center `}
    >
      <nav className='h-[5rem] bg-blue-500 w-full mt-3 flex rounded-lg flex-row justify-between items-center px-4'>
        <Link href='/'>
          <Image
            ...
            width={175}
            src={LogoImg}
            alt='yveslioneldiomande'
          />
        </Link>

        <div
          onClick={() => Toggle()}
          className={`items-center z-50 h-10 w-10 cursor-pointer rounded-full flex justify-center border-2 border-${
            show ? 'black' : 'white'
          }`}
        >
          {Navshow ? (
      ...
      <div
        ...
        } absolute  h-screen  bg-black w-full z-50 bg-opacity-80 flex justify-end`}
      >
        {/* Project Box */}
        <div
          className={`${
            animtation ? 'translate-x-[0rem]' : 'translate-x-[120rem]'
          } h-full transition duration-[400ms] ease-in-out delay-[10ms] flex bg-[#FFFDFC] w-[100%] md:w-[30%] flex-col`}
        >
          {/* Project Inofr */}
          <div className={`flex-1   overflow-y-scroll overflow-x-hidden p-3 `}>
            ...
            <div className='flex items-center px-4 border-b-gray-300 border-b-2 justify-between h-[3rem] '>
              <IoChevronBackCircleOutline
                ...
                color='gray'
              />
              <p className='text-md font-semibold text-black font-'>
                Retour aux projets
              </p>
            </div>
            {/* Title */}
            <p className='text-2xl mt-3 text-black font-semibold '>
              {Project ? Project?.title : 'Tiktok'}
            </p>
            {/* First Description */}
            <p className=' text-gray-400 text-sm '>
              {Project
                ? Project?.subtitle
                : 'designer positioned behind an Illustrator artboard adjusting pixels, but I design.'}
            </p>
            {/* Image */}
            <div className='h-[22rem] mt-3 rounded-lg  '>
              <Image
                src={Project ? Project?.preview : pro}
                alt='projet'
                className='h-[22rem] object-contain rounded-lg'
              />
            </div>
            {/* A propos */}
            ...
            {/* Project Status */}
            <p className=' text-black font-semibold mt-3 '>Statut</p>
            <div
            ...
            >
              {Project.status.toString() === '0' ? 'En cours' : 'Terminé'}
            </div>
            {/* TechNology */}
            ...
            <div className='flex flex-row items-center mt-4 gap-1 flex-wrap h-auto'>
              {Project ? (
                Project.technologies.map((tech, index) => (
                  <div
                    key={index}
                    className='h-[2.4rem] min-w-[4rem] p-3 bg-[#EFE9E4] items-center  justify-center flex rounded-lg'
                  >
            ...
            <Link
              className=' text-black my-2 font-semibold text-sm  '
              target='_blank'
              href={Project ? Project.openLink : 'https://piggment.co'}
            >
              {Project
                ? Project.openLink.substring(0, 100)
                : 'https://piggment.co'}
            </Link>
            {/* Github */}
            <div className='flex mt-3 gap-x-1 flex-row items-center'>
            ...
            <a className=' text-black my-2 font-semibold text-sm  '>
              {Project
                ? Project.githubLink
            ...
            <div className='flex mt-3 gap-x-1 flex-row items-center'>
              <FaAndroid />
              <p className=' text-black font-semibold  '>Apk</p>
            </div>
            {Project.type === 'mobile' && (
              <a className=' text-black my-2 font-semibold text-sm  '>
                {Project.apkLink}
          ...
          <Link
            target='_blank'
            href={Project ? Project.openLink : 'https://piggment.co'}
            className='h-[4rem] bg-blue-500 gap-x-4 hover:bg-blue-600 cursor-pointer flex justify-center items-center'
          >
      ...
      <h1 className='text-6xl mt-10 font-semibold'>
        ...
        {selectedId === 0
          ? projects.length
          : projects.filter((project) => project.projectType === selectedId)
              .length}
        )
      </h1>
      <div className='flex-row  flex gap-x-4 md:gap-x-6 mt-10 items-center'>
        {types.map((type) => (
          <div
            ...
            key={type.id}
            className={`cursor-pointer justify-center p-3 w-[4em]  md:w-[6rem] transition duration-[500ms] ease-in  flex items-center h-[2.4rem] ${
              selectedId === type.id ? 'bg-black' : ''
            } rounded-lg`}
          >
            <p
              className={` text-[1rem] md:text-lg transition duration-[300ms] ease-in-out ${
                selectedId === type.id
